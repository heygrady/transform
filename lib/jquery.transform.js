...
(function($, window, document, undefined) {
	...
	$.transform.prototype = {
		...
		applyMatrix: function(funcs) {
			...
			var translate = this.getAttr('translate', true),
				translateX = this.getAttr('translateX', true, true) || 0,
				translateY = this.getAttr('translateY', true, true) || 0;
		 ...
		 */
		getAttrs: function(split) {
			var attrs = {}, value;
			$.each($.transform.funcs, $.proxy(function (i, func) {
				value = this.getAttr(func, split, true);
				if (value || value === 0) {
					attrs[func] = value;
				}
			}, this));
		 ...
		 */
		getAttr: function(func, split, preserveNull) {
			//TODO: This function is kind of complicated. It should be simplified to always
			...
			// If we didn't find a value, we need to return a default.
